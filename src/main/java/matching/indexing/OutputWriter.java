...
class OutputWriter {
     ...
     *
     * @param docs Document object that to be added to the index.
     * @throws IOException
     */
    public void insert(Document[] docs) throws IOException {
        for (int fieldCtr = 0; fieldCtr < docs.length; fieldCtr++) {
            Document document = docs[fieldCtr];
            if (!document.getField("content").stringValue().equals("NULL")) {
