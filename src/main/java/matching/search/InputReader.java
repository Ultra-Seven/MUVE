...
class InputReader {
    private static final int TOTAL_HITS = 100;
    /**
    ...
    private int findTotalHits(Query query) throws IOException {
        ...
        TopDocs docs = this.searcher.search(query, 52051);
        for (int i = 0; i < 52051; i++) {
            System.out.println("Column: " +
                    Arrays.toString(Arrays.stream(this.searcher.doc(i).getValues("column")).toArray()) +
                    "\tContent: " +
                    Arrays.toString(Arrays.stream(this.searcher.doc(i).getValues("content")).toArray()));
        }
    ...
    private ScoreDoc[] findMatchingDocuments(Query query, int maxHits) throws IOException {
        TopScoreDocCollector docsCollector = TopScoreDocCollector.create(maxHits, TOTAL_HITS);
